...
use crate::{Result, U8Header, U8Node};

#[derive(Debug, Clone, Copy)]
pub struct Parser<T: Read + Seek>(T);
...
impl<T: Read + Seek> Parser<T> {
    ...
    pub fn read_bool(&mut self) -> Result<bool> {
        ...
        match byte[0] {
            0 => Ok(false),
            1 => Ok(true),
            _ => color_eyre::eyre::bail!("Invalid bool!"),
        }
    }
    ...
    pub fn read_string(&mut self, table_start: u32, table_offset: u32) -> Result<String> {
    ...
    pub fn read_u8_header<const MAGIC: u32>(&mut self) -> Result<U8Header> {
        let header = U8Header {
            magic: self.read()?,
        ...
        color_eyre::eyre::ensure!(header.magic == MAGIC.to_ne_bytes(), "incorrect u8 magic");
        Ok(header)
    }

    pub fn read_node(&mut self) -> Result<U8Node> {
        Ok(U8Node {
